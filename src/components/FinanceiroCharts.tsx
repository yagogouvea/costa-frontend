            ...
            {topClientes.map((cliente, index) => (
              <div key={cliente.nome} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                ...
                <div className="text-right">
                  <p className="font-bold text-sm text-green-600">{formatarValor(cliente.valor)}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <CardContent className="p-6">
          ...
          <div className="space-y-3">
            {topPrestadores.map((prestador) => (
              <div key={prestador.nome} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium text-sm">{prestador.nome}</p>
                    <p className="text-xs text-gray-500">{prestador.ocorrencias} ocorrências</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-bold text-sm text-blue-600">{formatarValor(prestador.valor)}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {dadosMensais.slice(-3).map((dado) => (
              <div key={dado.mes} className="p-4 bg-gray-50 rounded-lg">
                <h4 className="font-medium text-sm text-gray-600 mb-2">{dado.mes}</h4>
